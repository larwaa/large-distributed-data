...
class Importer:
    ...
    # After importing, this is `True` to avoid multiple imports of data
    imported = False
    ...
    def _import(self, df: pd.DataFrame, collection: Literal["users", "activities", "track_points"]):
        """
    ...
    def import_user_df(self) -> pd.DataFrame:
        ...
        user_directories = sorted([int(f) for f in os.listdir(self.data_path) if not f.startswith(".")])
        ...
        labeled_users = pd.read_csv(os.path.join(self.dataset_path, "labeled_ids.txt"), names=["_id"], dtype={"_id": np.int64}).sort_values("_id")
        
        # Add a `has_label` field for all users
        users_df["has_labels"] = users_df["_id"].isin(labeled_users["_id"])
        return users_df
    
    def _get_transportation_mode(self, users_df: pd.DataFrame, activities_df: pd.DataFrame) -> pd.DataFrame:
        """
        ...
        for user_id in users_df[users_df["has_labels"]]["_id"]:
            ...
            labels_file_path = os.path.join(self.data_path, id_with_leading_zeros, "labels.txt")
            labels_df = pd.read_csv(labels_file_path, skiprows=1, names=["start_datetime", "end_datetime", "transportation_mode"], parse_dates=["start_datetime", "end_datetime"], sep="\t")
            labels_df["user_id"] = user_id
            label_dfs.append(labels_df)

        labels_df = pd.concat(label_dfs)

        activities_df_copy = activities_df.merge(labels_df.drop_duplicates(["start_datetime", "end_datetime", "user_id"]), on=["start_datetime", "end_datetime", "user_id"], how="left")
        activities_df_copy["transportation_mode"] = activities_df_copy["transportation_mode"].fillna("")
        return activities_df_copy

    @timed
    def import_activities_and_track_points_df(self, users_df: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame]:
        """
        ...
        for user_id in users_df["_id"]:
            ...
            activity_path = os.path.join(self.data_path, id_with_leading_zeros, "Trajectory")
            ...
            for file_name in activity_file_names:
                ...
                with open(file_path, "r") as f:
                    if len(f.readlines()) <= self.activity_line_limit + self.HEADER_SIZE:
                        # Read the track points for an activity
                        df = pd.read_csv(
                            file_path,
                            skiprows=self.HEADER_SIZE,
                            names=["latitude", "longitude", "na", "altitude", "days_date", "date", "time"],
                            usecols=["latitude", "longitude", "altitude", "date", "time"],
                            dtype={"latitude": np.float64, "longitude": np.float64, "altitude": np.float64},
                        ...
                        activity_tuples.append((activity_id, user_id, df["datetime"].iloc[0], df["datetime"].iloc[-1]))
        ...
        activities_df = pd.DataFrame(activity_tuples, columns=["_id", "user_id", "start_datetime", "end_datetime"])
    ...
    def import_with_backreferences(self):
        ...
        """
        if self.imported:
            print("Already imported, run `wipe_collections` and try again")
            return
        
        users_df = self.import_user_df()
        activities_df, track_points_df = self.import_activities_and_track_points_df(users_df)
        ...
        user_activities = pd.DataFrame(activities_df.groupby(["user_id"])["_id"].apply(list).reset_index().rename({"_id": "activities", "user_id": "_id"}, axis=1))
        ...
        activity_track_points = pd.DataFrame(track_points_df.groupby(["activity_id"])["_id"].apply(list).reset_index().rename({"_id": "track_points", "activity_id": "_id"}, axis=1))
        ...
        activities_df["track_points"] = activities_df["track_points"].fillna("").apply(list)
