...
class Database(pydb.Database):
    ...
    def __getitem__(self, key: Literal["users", "track_points", "activities"] | str) -> Collection[User] | Collection[Activity] | Collection[TrackPoint] | Collection:
...
class CustomDbConnector(DbConnector):
    ...
    def __init__(self, host: str = "localhost", database: str = "mongo", user: str = "root", password: str = "password", port: int = 27017, connection_opts: str = "?authSource=admin"):
        super().__init__(DATABASE=f"{database}{connection_opts}", USER=user, PASSWORD=password, HOST=f"{host}:{port}")
